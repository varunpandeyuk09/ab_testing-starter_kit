...
### navigation (mobile drawer — verified)
- Hamburger trigger: `.pclBurger.mobileOnly a.mobileShowMenu` (`onclick="return mobileMenuToggle();"`), visible only ≤799px (mobile-only).
- Drawer: `#stMobileMenu` (has class `mobileOnly` → `display:none` on desktop), appended to `body` ONCE by `mobileMenuCreate()` in `fullwlibs.js` on the first toggle (`mobileMenuSlideToggle`). Inner container: `#stMobileMenu .stMobileMenuInner`.
- **Build order (the mechanism a reorder test must fight):** every `.mobileMenuContent` element's innerHTML is `prepend()`-ed into `.stMobileMenuInner` in DOM order → the drawer order is the REVERSE of DOM order. There are exactly two `.mobileMenuContent` sources (verified homepage + PLP): the footer `.pclFootMenu.noprint.mobileMenuContent` (`<h4>Links</h4>` + 12 links; its `.section2.mobileHide` "Shop" block is hidden on mobile) which is prepended LAST and therefore lands FIRST in the drawer, and `#pclLeftMenu.mobileMenuContent` (product categories) which lands LAST. Result in control: "Links" first, "Browse" second — products hidden below the fold.
- "Browse" block inside the drawer = `.menuHeading` whose trimmed text is `Browse` + its `nextElementSibling` `.browseMenu.pclMobileAllCats` (contains `#cat_tree`). On PLP pages `#pclLeftMenu` also carries a "Filter Results" narrow form (`#leftNarrowMenu`/`#narrowResultsForm`) BEFORE the Browse heading, so there are two `.menuHeading` nodes there.
- Drawer event bindings (narrow checkboxes etc.) are bound to the nodes themselves, so reordering `.stMobileMenuInner` children is safe. Since `mobileMenuCreate()` runs synchronously, a `MutationObserver` on `document.body` sees `#stMobileMenu` only after its innerHTML is fully built.
- User-confirmed (Q&A, TST Move products to the very top of the mobile nav): Browse goes to the very top (order: Browse → Links → Filter Results); no share.js tracking (KPIs are Convert platform metrics); QA skipped by user.
