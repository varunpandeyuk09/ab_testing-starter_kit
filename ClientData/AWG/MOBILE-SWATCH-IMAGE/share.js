(function () {
  try {
    ...
    function init() {
      // Goal 1: User taps a color swatch on mobile (image swap triggered)
      live('.val-color-swatches-variant', 'click', function (e) {
        var mq = window.matchMedia('(max-width: 767px)');
        if (!mq.matches) return;
        var color = this.getAttribute('data-property-color') || 'unknown';
        console.log('EG-AB045: Mobile swatch tapped — color: ' + color);
      });
    }
