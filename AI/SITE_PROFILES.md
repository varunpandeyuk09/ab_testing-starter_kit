...
### Reusable tooling
- `tools/qa_run.js` — `pcliq` SITES entry (empty `addToCart[]` → section flow). Spec-driven QA: `node tools/qa_run.js qa --spec "…/Product Listing Revamp/spec.json"`. Verified 12/12 PASS on PLP01 post-fix (Aug 2026). Spec `js` checks click a grade button and assert price + title changed on a MULTI-GRADE card, and that a disabled grade click is a no-op — behavioral checks fail (`vacuous pass blocked`) when no card can exercise the claim.
