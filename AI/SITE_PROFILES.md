...
### Reusable tooling
- `tools/qa_run.js` — `pcliq` SITES entry (empty `addToCart[]` → section flow). Spec-driven QA: `node tools/qa_run.js qa --spec "…/Product Listing Revamp/spec.json"`. Verified 13/13 PASS on PLP01 (Aug 2026). Spec `js` checks click a grade button and assert price + title changed on a MULTI-GRADE card, and that a disabled grade click is a no-op — behavioral checks fail (`vacuous pass blocked`) when no card can exercise the claim. A "every card has server data" check was deliberately NOT used: the PDP server stalls on bulk XHR fetches (verified via in-page async probes), so full coverage isn't deterministic cold — production uses an 8s abort timeout + retry. `settle.scrollAll` + async `js` checks (in-page fetch probes) were added to qa_run.js during this round.
