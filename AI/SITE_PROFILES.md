...
### Reusable tooling
- `tools/qa_run.js` — **reusable headed CDP QA runner (Node >=22, zero deps), 100% client-agnostic.** Auto-detects installed Chromium (Chrome/Edge/Brave/Opera/Vivaldi), drives headed via `--remote-debugging-port=0`, injects a `variation1/` dir into the live page, clicks Add to Cart, settles, runs golden assertions, writes a JSON report. Actions: `navigate|login|atc|qa`; flags `--browser --profile --url --inject --screenshot --out --wait-ms --headless --fresh`. Verified 13/13 PASS on the SM25 variation (Aug 2026). **No client code lives in the runner** — site facts (selectors/tokens) live in `AI/site_profiles.json` (data), and every check comes from the test's `spec.json`, so adding a client or a test never requires a qa_run.js edit. **Data-driven mode (the only QA mode):** pass `--spec <spec.json>` — the spec lives in the test folder and holds `settle` + all `checks` (ops `exists|not|eq|match|count|countLte|css|js|geom|batch`, tokens `{popup} {title} {update} {related} {relatedTitle} {relatedTile} {counter}` resolve from the site profile; fields `profile`/`url`/`inject` override CLI). The **`geom` op** (P34, added Aug 2026) asserts element geometry — `same-top/left-of/above/below/x-gap/y-gap/within/width-pct/centered/inside-viewport/equal-size` (+ `each:true` for grids) — so layout QA is deterministic from `getBoundingClientRect`, no screenshot eyeballing for facts. New tests need NO qa_run.js edits — just a profile entry + spec.json + variation files. Example: `node tools/qa_run.js qa --spec "…/TEST_NAME/AI_DATA/spec.json"`.
- `tools/cache_vision.js` + `tools/contract_to_spec.js` — **design contract pipeline (P35, added Aug 2026).** `cache_vision.js` hashes a STATIC reference image (SHA-256) + model + prompt-version into `hash@model@v<n>.json` under the test's `AI_DATA/vision_cache/` — MISS → process once + `--save`, HIT → reuse, never re-process. **Never cache control captures or live renders** (they change as code evolves). `contract_to_spec.js` turns the one-time Figma understanding (`AI_DATA/design_contract.json`: tokens → `css` ops, layout → `geom` ops, copy → `eq` ops) into the spec's design checks; behavioral checks (settle.*, js) are appended by hand. Ground truth stays the Figma/control images — the contract is a cache, and QA keeps one final vision pass as a safety net.
